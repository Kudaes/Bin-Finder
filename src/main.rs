...
use data::{FILE_PROCESS_IDS_USING_FILE_INFORMATION, PVOID};
use getopts::Options;
...
fn main() {
    unsafe {
        ...
        opts.optflag(
            "q",
            "quiet",
            "Reduces cross process activity by not resolving processes' names.",
        );
        ...
        match ret {
            Some(_x) => {}
            None => {
                println!("{}", &lc!("[x] EnumProcesses failed!"));
                return;
            }
        }
...
pub fn get_pid_from_image_path(path: &str) -> [usize; 500] {
    unsafe {
        ...
        if file_handle.0 == -1 {
            println!(
                "{}",
                &lc!("[x] The specified binary does not exist or can't be opened.")
            );
            return [0; 500];
        }
