...
use bindings::Windows::Win32::System::WindowsProgramming::{IO_STATUS_BLOCK, OBJECT_ATTRIBUTES};
use bindings::Windows::Win32::{
    Foundation::{HANDLE, HINSTANCE},
    System::Threading::{GetCurrentProcess, GetCurrentThread},
};
use data::{
    ApiSetNamespace, ApiSetNamespaceEntry, ApiSetValueEntry, EntryPoint, ExceptionHandleFunction,
    LptopLevelExceptionFilter, NtAllocateVirtualMemoryArgs, NtCreateThreadExArgs,
    NtOpenProcessArgs, NtProtectVirtualMemoryArgs, NtWriteVirtualMemoryArgs, PeMetadata, CLIENT_ID,
    CONTEXT, DLL_PROCESS_ATTACH, EAT, EXCEPTION_POINTERS, MEM_COMMIT, MEM_RESERVE,
    PAGE_EXECUTE_READ, PAGE_READONLY, PAGE_READWRITE, PROCESS_QUERY_LIMITED_INFORMATION,
    PS_ATTRIBUTE_LIST, PVOID,
};
use libc::c_void;
use litcrypt::lc;
use rand::Rng;
...
/// let instruction_addr = dinvoke::find_syscall_address(nt_open_process as usize);
...
pub fn set_hardware_breakpoint(address: usize) {
    unsafe {
        ...
        let mut lp_context: *mut bindings::Windows::Win32::System::Diagnostics::Debug::CONTEXT =
        ...
        (*context).Dr6 = 0;
        (*context).Dr7 &= !(((1 << 2) - 1) << 16);
        (*context).Dr7 &= !(((1 << 2) - 1) << 18);
...
/// let ntdll = dinvoke::get_module_base_address("ntdll.dll");
/// let nt_open_process = dinvoke::get_function_address(ntdll, "NtOpenProcess");
/// let syscall_addr = dinvoke::find_syscall_address(nt_open_process as usize);
/// ```
...
pub unsafe extern "system" fn breakpoint_handler(exceptioninfo: *mut EXCEPTION_POINTERS) -> i32 {
    if (*(*(exceptioninfo)).exception_record).ExceptionCode.0 == 0x80000004
    // STATUS_SINGLE_STEP
    {
        if ((*(*exceptioninfo).context_record).Dr7 & 1) == 1
            && (*(*exceptioninfo).context_record).Rip == (*(*exceptioninfo).context_record).Dr0
        {
            ...
            match HARDWARE_EXCEPTION_FUNCTION {
                ExceptionHandleFunction::NtAllocateVirtualMemory => {
                    (*(*exceptioninfo).context_record).R10 =
                        NT_ALLOCATE_VIRTUAL_MEMORY_ARGS.handle.0 as u64;
                    (*(*exceptioninfo).context_record).Rdx =
                        NT_ALLOCATE_VIRTUAL_MEMORY_ARGS.base_address as u64;
                }
                ExceptionHandleFunction::NtProtectVirtualMemory => {
                    (*(*exceptioninfo).context_record).R10 =
                        NT_PROTECT_VIRTUAL_MEMORY_ARGS.handle.0 as u64;
                    (*(*exceptioninfo).context_record).Rdx =
                        NT_PROTECT_VIRTUAL_MEMORY_ARGS.base_address as u64;
                    (*(*exceptioninfo).context_record).R8 =
                        NT_PROTECT_VIRTUAL_MEMORY_ARGS.size as u64;
                    (*(*exceptioninfo).context_record).R9 =
                        NT_PROTECT_VIRTUAL_MEMORY_ARGS.protection as u64;
                }
                ExceptionHandleFunction::NtOpenProcess => {
                    (*(*exceptioninfo).context_record).R10 = NT_OPEN_PROCESS_ARGS.handle as u64;
                    (*(*exceptioninfo).context_record).Rdx = NT_OPEN_PROCESS_ARGS.access as u64;
                    (*(*exceptioninfo).context_record).R8 = NT_OPEN_PROCESS_ARGS.attributes as u64;
                    (*(*exceptioninfo).context_record).R9 = NT_OPEN_PROCESS_ARGS.client_id as u64;
                }
                ExceptionHandleFunction::NtWriteVirtualMemory => {
                    (*(*exceptioninfo).context_record).R10 =
                        NT_WRITE_VIRTUAL_MEMORY_ARGS.handle.0 as u64;
                    (*(*exceptioninfo).context_record).Rdx =
                        NT_WRITE_VIRTUAL_MEMORY_ARGS.base_address as u64;
                    (*(*exceptioninfo).context_record).R8 =
                        NT_WRITE_VIRTUAL_MEMORY_ARGS.buffer as u64;
                    (*(*exceptioninfo).context_record).R9 =
                        NT_WRITE_VIRTUAL_MEMORY_ARGS.size as u64;
                }
                ExceptionHandleFunction::NtCreateThreadEx => {
                    (*(*exceptioninfo).context_record).R10 = NT_CREATE_THREAD_EX_ARGS.thread as u64;
                    (*(*exceptioninfo).context_record).Rdx = NT_CREATE_THREAD_EX_ARGS.access as u64;
                    (*(*exceptioninfo).context_record).R8 =
                        NT_CREATE_THREAD_EX_ARGS.attributes as u64;
                    (*(*exceptioninfo).context_record).R9 =
                        NT_CREATE_THREAD_EX_ARGS.process.0 as u64;
                }
...
pub fn get_function_address(module_base_address: isize, function: &str) -> isize {
    unsafe {
        ...
        for x in 0..number_of_names {
            let address =
                *((module_base_address + names_rva as isize + x as isize * 4) as *mut i32);
            ...
            if function_name.to_lowercase() == function.to_lowercase() {
                let function_ordinal = *((module_base_address
                    ...
                    + x as isize * 2) as *mut i16) as i32
                    + ordinal_base;
                let function_rva = *((module_base_address
                    + functions_rva as isize
                    + (4 * (function_ordinal - ordinal_base)) as isize)
                    as *mut i32);
...
pub fn get_api_mapping() -> HashMap<String, String> {
    unsafe {
        ...
        let process_information: *mut c_void =
            std::mem::transmute(&PROCESS_BASIC_INFORMATION::default());
        ...
        let api_set_namespace_ptr = *(((*process_information_ptr).PebBaseAddress as usize
            + api_set_map_offset) as *mut isize);
        let api_set_namespace_ptr: *mut ApiSetNamespace =
            std::mem::transmute(api_set_namespace_ptr);
        ...
        for i in 0..namespace.count {
            ...
            let mut api_set_entry_name_ptr =
            ...
            while j < (set_entry.name_length / 2) {
                ...
                if c != '\0'
                // Esto se podria meter en una funcion aparte
                {
            ...
            if set_entry.value_length == 1 {
                ...
                for x in 0..set_entry.value_length {
                    let host_ptr = (api_set_entry_name_ptr as usize
                        ...
                        + size_of::<ApiSetValueEntry>() * x as usize)
                        as *mut u8;
...
///     if id != -1
///     {
///         let addr = dinvoke::prepare_syscall(id as u32, Default::default());
...
pub fn prepare_syscall(id: u32, eat: EAT) -> isize {
    let mut sh: [u8; 21] = [
        0x4C, 0x8B, 0xD1, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x49, 0xBB, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x41, 0xFF, 0xE3,
    ];
...
pub fn call_module_entry_point(
    ...
    unsafe {
        ...
        let module = HINSTANCE(entry_point);
...
pub fn get_function_address_by_ordinal(module_base_address: isize, ordinal: u32) -> isize {
    ldr_get_procedure_address(module_base_address, "", ordinal)
}
        ...
        match ret {
            Some(x) => {
                if x != 0 {
                    ...
                    get_module_base_address(module)
                }
            }
            None => 0,
        }
...
pub fn nt_open_process(
    ...
    unsafe {
        ...
        if HARDWARE_BREAKPOINTS {
            ...
            let c = CLIENT_ID {
                unique_process: HANDLE(std::process::id() as isize),
                unique_thread: HANDLE::default(),
            };
